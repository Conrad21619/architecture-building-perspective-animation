...
- A simple preview window for testing guide placement interactively.
...
4. The resulting guides can be previewed in a simple window so you can tune the horizon, angle, and depth before using them in animation.
...
## Publishing notes
The repository is structured so it can be published as a simple open-source starter for a Krita perspective-guide workflow. The core logic is isolated in the Python module, making it straightforward to adapt into a full plugin with a richer visual UI later.